...
void build_psf_fft(int width, int height, float ffwhm_x, float ffwhm_y)
{
    ...
    Mat kernelX = getGaussianKernel(60, ffwhm_x);
    Mat kernelY = getGaussianKernel(60, ffwhm_y);
    ...
    cuda::multiply(gPSFR, psum, gPSFR);
    Mat complexPSF;
...
int run(){
    ...
    double snr = 0.0000001 + (alpha / 15000000.0);
    double ffwhm_x = 0.5 + fwhm_x / 230.0;
    double ffwhm_y = 0.5 + fwhm_y / 230.0;
